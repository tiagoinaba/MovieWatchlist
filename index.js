...
searchBtn.addEventListener('click', () => {
    fetch(`${apiUrl}s=${searchInput.value}`)
        .then(res => res.json())
        .then(data => {
            if(data.Response === 'True') {
            let html = ``
            for(let item of data.Search) {
                fetch(`${apiUrl}i=${item.imdbID}`)
                    .then(res => res.json())
                    .then(data => {
                        html += `
                        <div class="movie">
                            ...
                            <div class="info">
                                ...
                                <div class="mid">
                                    <p>${data.Runtime}</p>
                                    <p>${data.Genre}</p>
                                    <button data-movie-id="${data.imdbID}" data-movie-title="${data.Title}" class="add-to-watchlist-btn">
                                        ...
                                        <p data-movie-id="${data.imdbID}" data-movie-title="${data.Title}">Watchlist</p>
                                    </button>
                                </div>
                        ...
                        for(let item of addToWatchlistBtns) {
                            item.addEventListener('click', e => {
                                localStorage.setItem(e.target.dataset.movieId, e.target.dataset.movieTitle)
                            })
                        }
            ...
            document.querySelector('.container').innerHTML = `<h3>Unable to find what you're looking for. Please try another search.</h3>`
        }
